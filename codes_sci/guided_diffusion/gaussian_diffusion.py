    ...
    def p_sample(
        ...
        if conf.inpa_inj_sched_prev:
            ...
            if pred_xstart is not None:
                ...
                '''
                gt_= toU8(weighed_gt.cpu())
                noise_ = toU8(x.cpu())
                mask_ = toU8(gt_keep_mask.cpu())
                in_mask_ = toU8((1 - gt_keep_mask).cpu())
                mask_gt_ = toU8((gt_keep_mask * (weighed_gt)).cpu())
                mask_noise_ = toU8(((1 - gt_keep_mask) * (x)).cpu())
                x_ = toU8(x)

                Image.fromarray(gt_[0]).save(os.path.join(conf.save_dir, '{}_gt.png'.format(t.cpu().item())))
                Image.fromarray(noise_[0]).save(os.path.join(conf.save_dir, '{}_noise.png'.format(t.cpu().item())))
                Image.fromarray(mask_[0]).save(os.path.join(conf.save_dir, '{}_mask.png'.format(t.cpu().item())))
                Image.fromarray(in_mask_[0]).save(os.path.join(conf.save_dir, '{}_in_mask.png'.format(t.cpu().item())))
                Image.fromarray(mask_gt_[0]).save(os.path.join(conf.save_dir, '{}_mask_gt.png'.format(t.cpu().item())))
                Image.fromarray(mask_noise_[0]).save(os.path.join(conf.save_dir, '{}_mask_noise.png'.format(t.cpu().item())))
                Image.fromarray(x_[0]).save(os.path.join(conf.save_dir, '{}_x.png'.format(t.cpu().item())))
                '''
